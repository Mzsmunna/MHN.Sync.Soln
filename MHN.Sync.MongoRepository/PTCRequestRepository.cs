...
namespace MHN.Sync.MongoRepository
{
    public class PTCRequestRepository : RepositoryBase //, IPTCRequestRepository
    {
        ...
        static PTCRequestRepository()
        {
            if (!BsonClassMap.IsClassMapRegistered(typeof(PTCRequest)))
            {
                BsonClassMap.RegisterClassMap<PTCRequest>(map =>
                {
                    map.AutoMap();
        ...
        public PTCRequestRepository(IDatabaseContext dbContext)
            : base(dbContext)
        {
            try
            {
                if (Collection == null)
                {
                    Collection = _database.GetCollection<PTCRequest>("PTCRequest");
                }
                core = new MongoDBCore<PTCRequest>(Collection);
            }
            catch (Exception ex)
            {
                throw ex;
        ...
        private FilterDefinition<PTCRequest> BuildFilter(string _id)
        {
            var filter = Builders<PTCRequest>.Filter.Empty;

            if (!string.IsNullOrEmpty(_id) && _id.ToLower() != "undefined")
            {
                filter = filter & Builders<PTCRequest>.Filter.Eq("_id", _id);
            }

            return filter;
        }

        public async Task<PTCRequest> GetById(string _id)
        {
        ...
        public async Task<List<PTCRequest>> GetAllByField(string fieldName, string fieldValue)
        {
            var filter = Builders<PTCRequest>.Filter.Eq(fieldName, fieldValue);
        ...
        public async Task<List<PTCRequest>> GetAll()
        {
            var filter = BuildFilter(null);
            return await Collection.Find(filter).ToListAsync().ConfigureAwait(false);
        }

        public async Task<List<PTCRequest>> GetAll(int currentPage, int pageSize)
        {
            var filter = BuildFilter(null);
        ...
        public string Save(IEntity entity)
        {
            ...
            try
            {
                MongoDbOperationResult result = new MongoDBHelper<PTCRequest>(Collection).Save(entity).Result;
                returnVal = result.Id;
            }
            catch (Exception ex)
            {
        ...
        public string SaveMany(IEnumerable<PTCRequest> records)
        {
            var returnVal = string.Empty;
        ...
        public bool? Delete(IEntity entity)
        {
            ...
            try
            {
                PTCRequest _actualEntity = entity as PTCRequest;
                //_actualEntity.IsDeleted = true;
                //_actualEntity.ModifiedOn = DateTime.Now;

                MongoDbOperationResult result = new MongoDBHelper<PTCRequest>(Collection).Save(_actualEntity).Result;
